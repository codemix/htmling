...
module.exports = Classing.create('Template', {
   ...
   */
  escape: function(strValue) {
    if (String(strValue) === '0') { return 0; }
    if (!strValue) { return ''; }

    var isObject = (typeof strValue) === 'object';
    var safeHtml = isObject && strValue.hasOwnProperty('ownHtml');
    if (safeHtml) { return strValue.ownHtml; }

    return String(strValue).replace(/[&<>"']/g, function(match) {
      switch (match) {
        case '&': return '&amp;';
        case '<': return '&lt;';
        case '>': return '&gt;';
