...
ATTRIBUTES "Attribute List"
  = _ items:(ATTRIBUTE _)+ {
    return extractList(items, 0).reduce(function (attrs, attr) {
...
LogicalExpression
  = left:BinaryExpression _ op:("||" / "&&") _ right:Expression {
    return {
      type: 'LogicalExpression',
      operator: op,
      left: left,
...
SequenceExpression
  = "(" _ head:Expression? tail:(_ "," _ Expression)* _ ")" {
    var expressions = [];
    if (head) {
      expressions = [head].concat(extractList(tail, 3));
    }
...
ArrayExpression
  = "[" _ head:Expression? tail:(_ "," _ Expression)* _ "]" {
    var elements = [];
    if (head) {
      elements = [head].concat(extractList(tail, 3));
    }
    return {
...
ObjectExpression
  = "{" _ head:Property? tail:(_ "," _ Property)* _ "}" {
    var props = [];
    if (head) {
      props = [head].concat(extractList(tail, 3));
    }
    return {
...
Property
  = key:(Identifier / Literal) _ ":" _ value:Expression {
    return {
      type: 'Property',
      key: key,
      value: value
